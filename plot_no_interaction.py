...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cumulative_reward_runs",
        action="store_true", 
        help="Whether the cumulative reward was collected every run for plotting error bars on the performance plot."
    )
...
def main() -> None:
    ...
    with np.load(data_path) as data:
        ...
        if args.cumulative_reward_runs:
            cumulative_reward_runs = data["cumulative_reward_per_run"]
        else:
            cumulative_reward_runs = None
    ...
    for episode_idx, episode in enumerate(episodes):
        ...
        _apply_bold_3d_text(
            ax,
            rf"3D trajectories: $j={episode}$, $r_j = {radius_arr[episode_idx]:.3g}$ (m)",
        )
        if episode_idx == 0:
            ax.legend()
    ...
    for episode_idx, episode in enumerate(episodes):
        ...
        _apply_bold_3d_text(
            ax,
            rf"Agent {agent_idx} tube: $j={episode}$, $r_j = {radius_arr[episode_idx]:.3g}$ (m)",
        )
